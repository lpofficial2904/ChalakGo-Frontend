...
export default function Footer() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/settings`)
      ...
      .then((data) => data && setSettings((value) => ({ ...defaults, ...value, ...data })))
      .catch(() => {})
  }, [])
  useEffect(() => {
  ...
  const socialLinks = socialNetworks.map(([name, key]) => [
    name,
    key,
    settings[key] || (key === 'whatsapp' ? `https://wa.me/${phone.replace(/\D/g, '')}` : `https://${key}.com`),
  ])
  const whatsappLink = settings.whatsapp || `https://wa.me/${phone.replace(/\D/g, '')}`
    ...
    <footer className="overflow-hidden bg-[#071226] text-slate-300">
      <div className="mx-auto max-w-[1280px] px-5 py-14 sm:py-16 lg:px-8">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8">
          <div>
            ...
            <p className="mt-5 max-w-[16rem] text-sm leading-7 text-slate-400">
              Professional driver services for daily travel, business journeys and every special
              occasion.
            </p>
          ...
          <div>
            ...
            <div className="mt-5 grid gap-5 text-sm">
              ...
              />
              {settings.address && (
                <ContactItem icon={<MapPin size={18} />} title="Address" text={settings.address} />
              )}
              <ContactItem
                icon={<Clock3 size={18} />}
                title="Working hours"
        ...
        <div className="mt-14 flex flex-col gap-3 border-t border-white/10 pt-6 text-center text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:text-left">
          ...
          <span>
            Verified drivers <i className="px-1 text-blue-300">&bull;</i> Live location{' '}
            <i className="px-1 text-blue-300">&bull;</i> India-wide service
          </span>
        </div>
      </div>
...
function FooterColumn({ title, links, empty }) {
  return (
    <div>
      <h2 className="text-sm font-extrabold uppercase tracking-[.16em] text-white">{title}</h2>

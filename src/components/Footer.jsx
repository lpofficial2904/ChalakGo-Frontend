...
export default function Footer() {
  ...
  return <footer className="overflow-hidden bg-[#071226] text-slate-300"><div className="mx-auto max-w-[1280px] px-5 py-14 sm:py-16 lg:px-8">
    ...
    <div className="mt-14 flex flex-col gap-3 border-t border-white/10 pt-6 text-center text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between sm:text-left"><span>&copy; {new Date().getFullYear()} {settings.siteName}. All rights reserved.</span><span>Verified drivers <i className="px-1 text-blue-300">&bull;</i> Live location <i className="px-1 text-blue-300">&bull;</i> India-wide service</span></div>
  </div></footer>
}

function FooterColumn({ title, links, empty }) { return <div><h2 className="text-sm font-extrabold uppercase tracking-[.16em] text-white">{title}</h2><div className="mt-5 grid gap-3.5 text-sm">{links.map(([label, path]) => <Link key={path} className="flex w-fit items-center gap-2 transition hover:translate-x-1 hover:text-blue-300" to={path}><span className="text-blue-300">&rarr;</span>{label}</Link>)}{!links.length && <p className="text-slate-500">{empty}</p>}</div></div> }
function ContactItem({ icon, title, text, href }) { const content = <><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-blue-500/10 text-blue-300">{icon}</span><span><b className="block text-slate-100">{title}</b><span className="mt-1 block leading-5 text-slate-400">{text}</span></span></>; return href ? <a href={href} className="flex items-start gap-3 transition hover:text-white">{content}</a> : <div className="flex items-start gap-3">{content}</div> }

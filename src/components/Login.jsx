...
export default function Login() {
  ...
  const requestOtp = async (event) => {
    ...
    busy.current = true; setMessage(""); setLoading(true);
    const requested = { email: email.trim().toLowerCase(), signup: mode === "signup" };
    try {
      const response = await fetch(`${API_BASE}/api/users/otp/request`, { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...requested, ...(requested.signup ? { mobile, username } : {}) }) });
      const data = await readApiResponse(response); setChallenge(requested); setOtp(""); setStep("otp"); toast.success(data.message || "OTP sent to your email.");
    } catch (error) { setMessage(error.message || "Unable to send OTP."); toast.error(error.message || "Unable to send OTP."); } finally { busy.current = false; setLoading(false); }
  };
  const verifyOtp = async (event) => {
    ...
    busy.current = true; setMessage(""); setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/api/users/otp/verify`, { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...challenge, otp }) });
      finishLogin(await readApiResponse(response));
    } catch (error) { setMessage(error.message || "Unable to verify OTP."); toast.error(error.message || "Unable to verify OTP."); } finally { busy.current = false; setLoading(false); }
  };
  const switchMode = (next) => { if (busy.current) return; setChallenge(null); setMode(next); setStep("details"); setMessage(""); setOtp(""); };

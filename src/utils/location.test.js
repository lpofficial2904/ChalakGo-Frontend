...
test('maps house, building, street and administrative address fields', () => {
  const details = toLocationDetails({
    display_name: 'Complete returned address',
    address: {
      house_number: '7B',
...
test('rural and partial responses leave unavailable details blank', () => {
  const details = toLocationDetails({
    address: { village: 'Test Village', county: 'Test District', state: 'Test State' },
  })
...
test('does not mislabel a road name as an apartment name', () => {
  assert.equal(
    toLocationDetails({ name: 'Test Road', category: 'highway', address: { road: 'Test Road' } })
      .buildingName,
    ''
  )
  assert.equal(
...
test('edited address values are submitted with original GPS coordinates', () => {
  const form = {
    ...addressFormValues(
      toLocationDetails({ address: { road: 'Old Road', building: 'Old Building' } })
    ),
    address: 'Customer corrected address',
    houseNumber: '8',
    buildingName: 'Corrected Building',
    mainRoad: 'Corrected Road',
  }
  ...
  await assert.rejects(
    getFreshPosition({
      getCurrentPosition(success) {
        success({ coords: { latitude: 100, longitude: 1, accuracy: 1 }, timestamp: 123 })
      },
    }),
    /Invalid/
  )
...
test('improves an approximate first fix and stops when a precise fix arrives', async () => {
  ...
  let calls = 0
  const result = await getBestFreshPosition({
    getCurrentPosition(success, _error, options) {
      assert.equal(options.maximumAge, 0)
      assert.equal(options.enableHighAccuracy, true)
      success({
        coords: { latitude: 12 + calls / 100, longitude: 76, accuracy: accuracies[calls++] },
        timestamp: 123456 + calls,
      })
    },
  })
...
test('never replaces a better GPS fix with a worse reading or loops forever', async () => {
  ...
  const result = await getBestFreshPosition({
    getCurrentPosition(success) {
      success({
        coords: { latitude: 12, longitude: 76, accuracy: [200, 150, 900][calls++] },
        timestamp: 123456,
      })
    },
  })
...
test('keeps the available fix if subsequent refinement times out', async () => {
  ...
  const result = await getBestFreshPosition({
    getCurrentPosition(success, error) {
      if (calls++ === 0)
        success({ coords: { latitude: 12, longitude: 76, accuracy: 70 }, timestamp: 123456 })
      else error({ code: 3 })
    },
  })
  assert.equal(calls, 3)

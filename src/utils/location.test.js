...
test("rural and partial responses leave unavailable details blank", () => {
  ...
  assert.equal(
    toLocationDetails({ address: { county: "Test District" } }).city,
    "",
  );
...
test("edited address values are submitted with original GPS coordinates", () => {
  const form = {
    ...addressFormValues(
      toLocationDetails({
        address: { road: "Old Road", building: "Old Building" },
      }),
    ),
  ...
  await assert.rejects(
    getFreshPosition({
      getCurrentPosition(success) {
        success({
          coords: { latitude: 100, longitude: 1, accuracy: 1 },
          timestamp: 123,
        });
      },
    }),
...
test("improves an approximate first fix and stops when a precise fix arrives", async () => {
  ...
  const result = await getBestFreshPosition({
    getCurrentPosition(success, _error, options) {
      ...
      success({
        coords: {
          latitude: 12 + calls / 100,
          longitude: 76,
          accuracy: accuracies[calls++],
        },
...
test("good first reading needs no retry and permission denial stops immediately", async () => {
  ...
  await getBestFreshPosition({
    getCurrentPosition(success) {
      calls++;
      success({
        coords: { latitude: 12, longitude: 76, accuracy: 10 },
        timestamp: 123456,
      });
    },
  });
...
test("keeps the available fix if subsequent refinement times out", async () => {
  ...
  const result = await getBestFreshPosition({
    getCurrentPosition(success, error) {
      if (calls++ === 0)
        success({
          coords: { latitude: 12, longitude: 76, accuracy: 70 },
          timestamp: 123456,
        });
      else error({ code: 3 });
    },

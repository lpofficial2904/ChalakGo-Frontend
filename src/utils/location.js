...
export function toLocationDetails(data) {
  if (
    !data ||
    data.error ||
    (!data.display_name && !Object.keys(data.address || {}).length)
  )
    throw new Error("No address found for this location");
  const address = data.address || {};
  const buildingName =
    value(address, "building", "house_name", "apartments", "residential") ||
    (["building", "amenity", "office", "shop"].includes(
  ...
  details.formattedAddress =
    ...
    [...new Set(Object.values(details).filter(Boolean))].join(", ");
  return details;
}
...
// Booking forms should feel responsive. A recent network/GPS fix is normally
// sufficient for choosing a pickup; the customer can still refresh for a new fix.
export function getQuickPosition(geolocation, signal) {
  return getFreshPosition(geolocation, signal, 3000, {
    enableHighAccuracy: false,
    maximumAge: 300000,
  });
}

export async function getBestFreshPosition(
  geolocation,
  signal,
  onProgress = () => {},
  fast = false,
) {
  if (fast) return getQuickPosition(geolocation, signal);
  let best = null;
  let lastError;
  // Used where precision matters; booking screens use the fast path above.
  ...
  for (let attempt = 0; attempt < 3 && Date.now() < deadline; attempt++) {
    ...
    try {
      ...
      if (!best || reading.coordinates.accuracy <= best.coordinates.accuracy)
        best = reading;
